...
def build_prompt_based_filename(prompt_text: str) -> str:
    ...
    return f"user_wide_03_{prompt_component}_{unique_suffix}.png"
...
def upload_image_to_gcs(
    ...
    try:
        ...
        if object_name:
            ...
            filename = f"user03/{cleaned_object_name}"
        else:
            timestamp = datetime.datetime.utcnow().strftime("%Y%m%d_%H%M%S")
            filename = f"user03/{filename_prefix}_{timestamp}_{uuid.uuid4().hex}.png"
